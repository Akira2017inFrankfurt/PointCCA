...
from psg_model_3 import SimAttn_Seg
...
def init_process(rank, local_world_size, args):
    # 初始化各进程环境 start
...
def get_sampler_and_dataloader(rank, args):
    train_data_set = ShapeNetPart(
        root=args.root,
        partition='trainval',
        num_points=2048,
        class_choice=args.class_choice
    )
    if len(train_data_set) < 100:
        drop_last = False
    else:
        drop_last = True

    # test_data_set = ShapeNetPart(
    #     root=args.root,
    #     partition='test',
    #     num_points=2048,
    #     class_choice=args.class_choice)
...
def main_fn(rank, local_world_size, args):
    checkpoint_path, rank, device = init_process(rank, local_world_size, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', type=str, default=r'/home/haruki/下载/shapenet/shapenet_part_seg_hdf5_data')
